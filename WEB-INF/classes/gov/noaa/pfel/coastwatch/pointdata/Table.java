  ...
  public void readNcCF(
    ...
    try (NetcdfFile ncFile = NcHelper.openFile(fullName)) {
      ...
      */
      ...
      Dimension outerDimDim = outerDim < 0 ? null : dimsList.get(outerDim);
      Dimension innerDimDim = innerDim < 0 ? null : dimsList.get(innerDim);
      Dimension obsDimDim = obsDim < 0 ? null : dimsList.get(obsDim);
        ...
        if (innerDim < 0 || outerDim < 0)
          throw new SimpleException(
              errorInMethod
                  ...
                  + " obsDim="
                  + obsDim);
        obsDimDim = dimsList.get(obsDim);
        obsDimName = obsDimDim.getName();
        obsDimSize = obsDimDim.getLength();
        innerDimDim = dimsList.get(innerDim);
